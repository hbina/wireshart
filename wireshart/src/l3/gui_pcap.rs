use packet_parser::{PacketInfo, PcapPointer};
use std::fs::File;
...
fn create_pcap_block_widget_expanded<'a>(
    ...
    let parsed_packet = match packet_parser::parse_from_pcap_pointer(r, pcap_file) {
        Some(packet) => packet,
        None => {
    ...
    match parsed_packet.info {
        PacketInfo::Udp(udp) => {
            ...
            info_widget = info_widget.push(iced::widget::text!(
                "Destination Port: {}",
        ...
        PacketInfo::Tcp(tcp) => {
            ...
            info_widget =
                info_widget.push(iced::widget::text!("Urgent Pointer: {}", tcp.urgent_ptr));
        }
    }
